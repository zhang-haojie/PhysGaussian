...
def load_grouping_checkpoint(model_path, sh_degree=3, iteration=-1):
    # Find checkpoint
    ...
    classifier = torch.nn.Conv2d(gaussians.num_objects, 256, kernel_size=1)
    classifier.cuda()
    classifier.load_state_dict(torch.load(
        os.path.join(checkpt_dir, f"iteration_{iteration}", "classifier.pth")))

    return gaussians, classifier
...
def get_semantic_mask(objects_dc, classifier, selected_obj_ids, removal_thresh=0.5):
...
def auto_set_mpm(transformed_pos, bc_params):
    ...
    max_xyz = transformed_pos.max(dim=0)[0]
    center_xyz = ((max_xyz + min_xyz) / 2)

    size_xyz = (max_xyz - min_xyz)

    # 取 xyz 各分量
    size_x = size_xyz[0].item()
    size_y = size_xyz[1].item()
    size_z = size_xyz[2].item()
    center_x = center_xyz[0].item()
    center_y = center_xyz[1].item()
    center_z = center_xyz[2].item()
    y_min = min_xyz[1].item()
    y_max = max_xyz[1].item()

    thickness = 0.05     # 上下边界层厚度，也可根据实际自动推定
    v_up = 0.2           # 向上的速度
    v_down = -0.2        # 向下的速度
    t0 = 0.0             # 起始时间
    t1 = 1e3             # 终止时间（上边界施加很久）
...
if __name__ == "__main__":
    ...
    objects_dc = gaussians._objects_dc[mask, :, :]
